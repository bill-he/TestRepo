...
public class myServer extends Thread{
	...
	**/
	...
	volatile HashMap<String,myHttpServletSession> sessions = new HashMap<String,myHttpServletSession>();
	...
	myBlockingQueue sock_q = new myBlockingQueue(2000); // 2000 backlog for stress testing purposes
	
	//Servlet Session, context and Map of Servlet Objects.
	HashMap<String,HttpServlet> servlets;
	
	errorWriter errorlog_writer; 
	...
	public class errorWriter {
		...
		public errorWriter(File err ) throws IOException{
		...
		public synchronized void writeErrorLog(String str){
			log.debug("in errorWriter's WriteErrorLog");
			...
			try {
				...
				log.debug("errorWriter error message: " + str);
		...
		public synchronized void writeException(Exception e)  {
			log.debug("in errorWriter's WriteException");
			// file print writer
			...
			try {
				...
				// print writer for the exception stack trace conversion
				Writer writer = new StringWriter();
				PrintWriter printWriter = new PrintWriter(writer);
				e.printStackTrace(printWriter);
				String s = writer.toString();
				
				log.debug("errorWriter's Exception Message: " + s);
		...
		public synchronized String readErrorLog(){
			
			log.debug("in errorWriter's READ ERROR LOG");
			StringBuffer err  = new StringBuffer();

			try {

				List<String> lines = Files.readAllLines(errfile.toPath(), Charset.forName("UTF-8"));

				for(String line : lines ){
					//err.append("<p>").append(line).append("</p>");
					err.append(line);
			...
			log.debug("TO STRING" + err.toString());
			return err.toString();
	...
	public myServer(int p, String r, String web){
		...
		try{
			...
			log.debug(" creating log file in: " + root_dir + "/errorlog.txt");
			String errfilename = "errorlog.txt";
			File e = new File(errfilename);
			
			e.createNewFile();
			
			errorlog_writer = new errorWriter( e);
	...
	static class Handler extends DefaultHandler {
		...
		public void characters(char[] ch, int start, int length) {
			...
			if (m_state == 1) { // servlet name
				...
				m_state = 0;
			} else if (m_state == 6){ // servlet context name
				m_servletContextName = value;
				m_state = 0;
			}
		...
		private String m_servletContextName;
	...
	private static myServletContext createContext(Handler h, String root_dir) {
		myServletContext fc = new myServletContext(root_dir, h.m_servletContextName);
	...
	public void run(){
		...
		//shared accross all servlets... need to synchronize for concurrency
		...
		try {
			...
			*/
			...
			// TODO Auto-generated catch block
			errorlog_writer.writeErrorLog(e1.toString());
			e1.printStackTrace();
		...
		try {
		    ...
		    while (true){
		    	...
		    	try{
		    		if(shutdown.shutdown_requested == false){
		    			...
		    			synchronized(sock_q){
		    				...
		    				try {
								...
								// TODO Auto-generated catch block
								errorlog_writer.writeErrorLog(e.toString());
								e.printStackTrace();
							}
		    		...
		    		//errorlog_writer.writeErrorLog("Socket timed out!");
		    	}
		    }		    
		...
		for( HttpServlet s : servlets.values() ){
			log.debug("destroying servlet: " + s.getServletName());
			s.destroy();
		}
